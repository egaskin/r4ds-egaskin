...
if (!file.exists("./sessionInfo.txt")) {
  writeLines(capture.output(sessionInfo()), "sessionInfo.txt")
  print("Made new sessionInfo.txt")
} else{
  print("Kept existing sessionInfo.txt")
}
```
...
ggplot(mpg, aes(x = year, y = hwy, color = manufacturer)) +
  # geom_line() # connects observations in the order which they appear on the x axis. NOT line graph
  geom_smooth() # fits a line (with `color = manufacturer` specified, each unique value of manufacturer gets a line)
...
```{r}
# create a single bar sacked bar, so when it's converted there is only 1 color per each slice of the pie chart
